...
-- Add/modify columns 
alter table USUARIO add TEMPORIZADOR_DASH NUMBER(10);
-- Add comments to the columns 
comment on column USUARIO.TEMPORIZADOR_DASH
  is 'Determina o tempo entre a troca de tabs do dashboard';

-- Add/modify columns 
alter table CONFIGURACOES add APRES_TEMP_DASH VARCHAR2(1);

-- Add comments to the columns 
comment on column CONFIGURACOES.APRES_TEMP_DASH 
  is 'Essa coluna determina se o temporizador será apresentado na tela de dashboard';

alter table CONFIGURACOES add PORCENT_TEMP_DASH NUMBER(10);
-- Add comments to the columns 
comment on column CONFIGURACOES.PORCENT_TEMP_DASH  
  is 'Essa coluna determina a porcentagem da execução da tab atual que irá chamar a consulta da próxima tab(background)';

alter table CONFIGURACOES add INICIAL_TEMP_DASH VARCHAR2(1);
-- Add comments to the columns 
comment on column CONFIGURACOES.INICIAL_TEMP_DASH   
  is 'Essa coluna determina se o temporizador irá iniciar diretamente ou se o usuário deverá clicar em play';
  
-- Create table
create table ATR_MATRIZ_FORM_ESTADO (
  ATRIBUTO_ID         NUMBER(10) not null,
  ESTADO_ID           NUMBER(10) not null,
  FORMULARIO_ID       NUMBER(10) not null,
  ATRIBUTO_RELACIONADO_ID  NUMBER(10) not null,
  CAMPO_BLOQUEADO     VARCHAR2(1),
  CAMPO_INVISIVEL     VARCHAR2(1),
constraint PK_ATR_MATRIZ_FORM_ESTADO primary key (ATRIBUTO_ID, ESTADO_ID, FORMULARIO_ID, ATRIBUTO_RELACIONADO_ID)
  using index tablespace &CS_TBL_IND
)tablespace &CS_TBL_DAT;

COMMENT ON TABLE ATR_MATRIZ_FORM_ESTADO
    IS 'Tabela que indica se um atributo relacionado a outro atributo(Matriz) será oculdo ou bloqueado por estado.';

ALTER TABLE ATR_MATRIZ_FORM_ESTADO ADD CONSTRAINT FK_ATR_MATRIZ_FORM_ESTADO_01 
  FOREIGN KEY (FORMULARIO_ID) REFERENCES FORMULARIO (FORMULARIO_ID) ON DELETE cascade;
ALTER TABLE ATR_MATRIZ_FORM_ESTADO ADD CONSTRAINT FK_ATR_MATRIZ_FORM_ESTADO_02 
  FOREIGN KEY (ESTADO_ID, FORMULARIO_ID) REFERENCES ESTADO_FORMULARIO (ESTADO_ID, FORMULARIO_ID) ON DELETE cascade;
ALTER TABLE ATR_MATRIZ_FORM_ESTADO ADD CONSTRAINT FK_ATR_MATRIZ_FORM_ESTADO_03 
  FOREIGN KEY (ATRIBUTO_ID) REFERENCES ATRIBUTO (ATRIBUTOID) ON DELETE cascade;
ALTER TABLE ATR_MATRIZ_FORM_ESTADO ADD CONSTRAINT FK_ATR_MATRIZ_FORM_ESTADO_04 
  FOREIGN KEY (ATRIBUTO_RELACIONADO_ID) REFERENCES ATRIBUTO (ATRIBUTOID) ON DELETE cascade;
  
-- MV!!!
create materialized view log on tarefa 
       with rowid (datainicio,prazoprevisto,prazorealizado)
       including new values;
       
create materialized view log on responsavelentidade 
       with rowid 
       including new values;
       
create materialized view log on hora_planejada 
       with rowid, (tarefa_id,usuario_id,data,horas_planejadas,situacao)
       including new values;
       
create materialized view log on horatrabalhada 
       with rowid, (tarefa,responsavel,datatrabalho,minutos)
       including new values;
       
create materialized view log on hora_alocada 
       with rowid, (tarefa_id,data,minutos)
       including new values;

create materialized view log on dia 
       with rowid
       including new values;
       
create or replace force view v_dias_futuro as 
select data as dia from dia;

--Materialized View Horas Alocadas/Trabalhadas/Planejadas
CREATE MATERIALIZED VIEW MV_HORAS_ALTBPL REFRESH FAST ON COMMIT AS (
select t.rowid crid, r.rowid srid , f.rowid drid,
       id tarefa_id, r.responsavel USUARIO_ID, f.data,
       0 MINUTOS, NULL HORA_ID,
       NULL SITUACAO, 'HPREV' TIPO, 1 umarker
from tarefa t,
     responsavelentidade r,
     dia f
where least(t.datainicio, nvl(t.iniciorealizado, t.datainicio)) <= f.data
and   greatest(t.prazoprevisto, nvl(t.prazorealizado, t.prazoprevisto)) >= f.data and
      t.id = r.identidade
and   'T' = r.tipoentidade 
) union all (
select hp.rowid crid, null srid, null drid, hp.tarefa_id TAREFA_ID, hp.usuario_id USUARIO_ID, hp.data DATA,
       hp.horas_planejadas MINUTOS, hp.id HORA_ID,
       hp.situacao SITUACAO, 'HP' TIPO, 2 umarker
  from hora_planejada hp
) union all (
select ht.rowid crid, null srid, null drid, ht.tarefa TAREFA_ID, ht.responsavel, ht.datatrabalho,
       ht.minutos, ht.id, null, 'HT' TIPO, 3 umarker
  from horatrabalhada ht
) union all (
select ha.rowid crid, re.rowid srid, null drid, ha.tarefa_id, re.responsavel, ha.data,
       ha.minutos, ha.id, null, 'HA' TIPO, 4 marker
...
create or replace package pck_sla_nivel is

   type t_rec_sla_nivel is record (row_id rowid,
                                   id     sla_nivel.id%type);

   type tt_array_sla_nivel is table of t_rec_sla_nivel index by binary_integer;

   gt_registros_alterados tt_array_sla_nivel;
   gt_array_vazio         tt_array_sla_nivel;

end;
/
create or replace package body pck_sla_nivel is
...
create or replace package body pck_sla is
...
create or replace package pck_condicional is
      ...
      procedure p_ExecutaAcaoCondicional (rec_demanda in out nocopy demanda%rowtype, pprojetos in out nocopy tab_projeto, acao acao_condicional%rowtype, pv_usuario varchar2, pn_estado_id in out number, pn_estado_mensagem_id in out number, pn_enviar_email in out number, pn_gerar_baseline in out number, pv_msg_erro in out varchar2);   
...
create or replace package body pck_condicional is
   ...
   procedure p_ExecutaAcaoCondicional (rec_demanda in out nocopy demanda%rowtype, pprojetos in out nocopy tab_projeto, acao acao_condicional%rowtype, pv_usuario varchar2, pn_estado_id in out number, pn_estado_mensagem_id in out number, pn_enviar_email in out number, pn_gerar_baseline in out number) is
     lv_msg_erro varchar2(1000); 
   begin
     lv_msg_erro := ''; 
     p_ExecutaAcaoCondicional (rec_demanda, pprojetos, acao, pv_usuario, pn_estado_id, pn_estado_mensagem_id, pn_enviar_email, pn_gerar_baseline, lv_msg_erro);
   end;
     
   procedure p_ExecutaAcaoCondicional (rec_demanda in out nocopy demanda%rowtype, pprojetos in out nocopy tab_projeto, acao acao_condicional%rowtype, pv_usuario varchar2, pn_estado_id in out number, pn_estado_mensagem_id in out number, pn_enviar_email in out number, pn_gerar_baseline in out number, pv_msg_erro in out varchar2) is
   ...
   begin
        ...
        if 'GB' = upper(acao.acao) then
           ...
           if 'PF' = upper(acao.acao) then
              ...
              lv_valor_troca := to_char(ln_total, '99999999999999990D9999999999999999');
          ...
          if acao.propriedade_id is not null then
           ...
           if acao.campo_sla = 'SLA_PROCESSO' then
              ...
              pck_regras.p_copia_propriedade(rec_demanda.demanda_id,
               ...
               pv_msg_erro := lv_msg_erro;
         elsif 'EL' = upper(acao.acao) then   
              --Estornar Lançamentos
              lv_msg_erro := '';
              pck_regras.p_rest_estor_propriedade(rec_demanda.demanda_id,
                                             pv_usuario,
                                             acao.propriedade_id,
                                             'E',
                                             lv_msg_erro);
      ...
      if substr(upper(rec_campose.chave_campo), 1, 9) = 'ATRIBUTO_' and 
         ...
         if pck_atributo.Tipo_LISTA_MULTIPLA_ESCOLHA = rec_atributo.tipo or
            ...
            if 'IG' = rec_campose.condicional then
               ...
               if (lv_valor1 = lv_valor2 or (lv_valor1 is null and lv_valor2 is null)) then
                  return true;
               else
                  return false;
               end if;
...
create or replace package pck_regras is
 ...
 procedure p_rest_estor_propriedade (pn_demanda_id demanda.demanda_id%type, pv_usuario_id usuario.usuarioid%type, pn_propriedade_id_destino regras_propriedade.id%type, pn_situacao varchar2,pn_msg_erro_lanc out varchar2);
end pck_regras;
/
   ...
   */
   ...
   begin
     ...
     if (ln_aux=0) then
         ...
         for c in (select ep.nome_tabela p_nome_tabela, ep.coluna_pk p_coluna_pk, ep.id p_tipo_entidade_id,
                          ...
                          (select count(id) from regras_prop_nivel_item it where it.nivel_id = n.id) qtd_sub_niveis,
                          COUNT(*) OVER () total, 
           ...
           if pb_get_update and qtd_sub_niveis > 1 then
    ...
    /*Alerta quando copiar > 4000 char*/'      when dbms_lob.getlength([ALIAS-TAB-ATRIB].valor_html) > 4000 then to_char(to_number(''campo html grande demais. Gera erro.''))' ||
     ...
     --retorna o ID da lista
        ...
        if lv_ultimo_tipo_valor in ('atributo','lancamento') or lv_coluna_pk is null then
   ...
   begin
      ...
      if pn_propriedade_id_destino is not null then
        ...
        if ln_tipo_propriedade_id is null then
           select distinct tipo_propriedade_id, null 
           into ln_tipo_propriedade_id, ln_atributo_id 
           from regras_prop_nivel_item rni 
           where rni.nivel_id = (select n.id
                                 from regras_propriedade_niveis n
                                 where n.propriedade_id = pn_propriedade_id_destino
                                  and   ordem = (select max(ordem)
                                                 from regras_propriedade_niveis n2
      ...
      if lv_escopo not in ('demandaCorrente','demandasFilhas','demandasIrmas',
                           ...
                           'demandasIrmasMaisCorrente','demandasProjetosAssociadosMaisCorrente', 
                           'demandaPai') then
...
--             nvl(CONFIG.obrigatorio, CONFIGPADRAO.obrigatorio) obrigatorio
   ...
   procedure p_exec_regras_valid_trans ( pn_demanda_id demanda.demanda_id%type,
    ...
    ln_historico                                 number;
    lv_retorno_validacao_campos                  varchar2(4000); 
...
--Utilizado para restaurar ou estornar determinados lançamentos vinculados
--a demanda. É uma ação das regras condicionais     
procedure p_rest_estor_propriedade (pn_demanda_id demanda.demanda_id%type,
                                     pv_usuario_id usuario.usuarioid%type,
                                     pn_propriedade_id_destino regras_propriedade.id%type,
                                     pn_situacao varchar2,
                                     pn_msg_erro_lanc out varchar2) is
   ...
   begin
      ...
      if pn_propriedade_id_destino is not null then
         lv_valor_origem := f_get_valor_propriedade(pn_demanda_id, pv_usuario_id, pn_propriedade_id_destino);
      end if;
      
      if pn_propriedade_id_destino is not null then
        
        select tipo_propriedade_id
        into ln_tipo_propriedade_id
        from regras_propriedade_niveis n
        where n.propriedade_id = pn_propriedade_id_destino
        and   ordem = (select max(ordem)
                       from regras_propriedade_niveis n2
        ...
        if ln_tipo_propriedade_id is null then
           select distinct tipo_propriedade_id 
           into ln_tipo_propriedade_id 
           from regras_prop_nivel_item rni 
           where rni.nivel_id = (select n.id
                                 from regras_propriedade_niveis n
      ...
      for c in (select e.nome_tabela,
         ...
         while true loop
            fetch lc_valores into lv_valor_origem, ln_total, ln_linha;
            exit when lc_valores%notfound;
                 if c.tipo_valor = 'lancamento' and lv_valor_origem is not null and pn_situacao is not null then
                      lv_sql_lancamentos := ' update custo_lancamento ' ||
                                            ' set situacao = '''||pn_situacao||''' ' ||
                                            ' where id in ('||f_get_numero(lv_valor_origem)|| ')'; 
                      execute immediate lv_sql_lancamentos;
                 end if;
         end loop;
         close lc_valores;
      end loop;
...
BEGIN
   if (inserting) then
        ...
        pck_estado_formulario.gt_registros_alterados(pck_estado_formulario.gt_registros_alterados.count).formulario_id  := :new.formulario_id;
        pck_estado_formulario.gt_registros_alterados(pck_estado_formulario.gt_registros_alterados.count).estado_id  := :new.estado_id;
        pck_estado_formulario.gt_registros_alterados(pck_estado_formulario.gt_registros_alterados.count).sla_default_id  := :new.sla_default_id;
...
CREATE OR REPLACE TRIGGER trg_sla_nivel_iud_ar
AFTER DELETE OR INSERT OR UPDATE
      ON sla_nivel
FOR EACH ROW
DECLARE
...
BEGIN
   if (inserting) then
        pck_sla_nivel.gt_registros_alterados(pck_sla_nivel.gt_registros_alterados.count + 1).row_id  := :new.rowid;
        pck_sla_nivel.gt_registros_alterados(pck_sla_nivel.gt_registros_alterados.count + 1).id  := :new.id;        
   elsif (updating) or (deleting) then
        pck_sla_nivel.gt_registros_alterados(pck_sla_nivel.gt_registros_alterados.count + 1).row_id  := :old.rowid;
        pck_sla_nivel.gt_registros_alterados(pck_sla_nivel.gt_registros_alterados.count + 1).id  := :old.id;        
   end if;

END trg_sla_nivel_iud_ar;
/

CREATE OR REPLACE TRIGGER trg_sla_nivel_iud_as
AFTER DELETE OR INSERT OR UPDATE
      ON sla_nivel
DECLARE
ln_sla sla.id%type;
BEGIN
for i in 1 .. pck_sla_nivel.gt_registros_alterados.count loop
  ...
  /*
   begin
      ...
      into ln_sla
      from sla_nivel
      where rowid = pck_sla_nivel.gt_registros_alterados(i).row_id;
   exception
   when no_data_found then
      ln_sla := null;
   end;
  */
  ln_sla := pck_sla_nivel.gt_registros_alterados(i).id;
  
   for c in (select d.demanda_id
        from (select formulario_id, estado_id
              from estado_formulario ef
              where estado_final = 'N' or estado_final is null) ef,
             demanda d,
             sla_ativo_demanda a
        where ef.formulario_id = d.formulario_id
        and   ef.estado_id = d.situacao
        and   d.demanda_id = a.demanda_id
        and   (a.sla_processo_id = ln_sla or
               a.sla_tendencia_id = ln_sla or
               a.sla_estado_id = ln_sla or
             --  ln_sla is null or
               d.formulario_id in (select formulario_id
                                   from estado_formulario
                                   where sla_default_id = ln_sla))
        for update) loop

       pck_sla.pCalcula (c.demanda_id, true, true, true);

   end loop;

end loop;
END trg_sla_nivel_iud_as;
/

CREATE OR REPLACE TRIGGER trg_sla_iud_ar
AFTER DELETE OR INSERT OR UPDATE
	  OF qtd_minutos, atributo_id
      ON sla
FOR EACH ROW
DECLARE
...
BEGIN
   if (inserting) then
        pck_sla.gt_registros_alterados(pck_sla.gt_registros_alterados.count + 1).row_id  := :new.rowid;
        pck_sla.gt_registros_alterados(pck_sla.gt_registros_alterados.count + 1).id  := :new.id;        
   elsif (updating) or (deleting) then
        pck_sla.gt_registros_alterados(pck_sla.gt_registros_alterados.count + 1).row_id  := :old.rowid;
        pck_sla.gt_registros_alterados(pck_sla.gt_registros_alterados.count + 1).id  := :old.id;
   end if;

END trg_sla_iud_ar;
/

CREATE OR REPLACE TRIGGER trg_sla_iud_as
AFTER DELETE OR INSERT OR UPDATE
	  OF qtd_minutos, atributo_id
      ON sla
DECLARE
ln_sla sla.id%type;
BEGIN
for i in 1 .. pck_sla.gt_registros_alterados.count loop
...
/*
   ...
   into ln_sla
   from sla
   where rowid = pck_sla.gt_registros_alterados(i).row_id;
   exception
   when others then
      ln_sla := null;
   end;
*/
   ln_sla := pck_sla.gt_registros_alterados(i).id;
   ...
   for c in (select d.demanda_id
       ...
       update sla_ativo_demanda sad
       set qtd_minutos_critico = pck_sla.f_restante_critico(c.demanda_id)
       where sad.demanda_id = c.demanda_id;
...
insert into versao(id, titulo, patch, versao_tgp_id, comentario)
       values (versao_seq.nextval, '6.0.1', '5', 4, 'Aplicação de patch');
commit;
/
