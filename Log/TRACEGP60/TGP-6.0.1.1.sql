...
-------------------------------------------------------------------------------
-- Alteração de objetos existentes
-------------------------------------------------------------------------------

alter table NOTIFICACAO modify TIPO VARCHAR2(3);

-------------------------------------------------------------------------------
-- Novos objetos
-------------------------------------------------------------------------------

create table H_SEQUENCIA (
  ID              number(10)    not null,    
  ATRIBUTO_ID     number(10)    not NULL,
  NOME_COLUNA     varchar2(50),
  VALOR_ANTERIOR  number(10),
  VALOR_ATUAL 	  number(10),
  USER_UPDATE     varchar2(50),
  DATE_UPDATE     date          default sysdate not null,
constraint PK_H_SEQUENCIA primary key (id) using index tablespace &CS_TBL_IND
) tablespace &CS_TBL_DAT;

create sequence H_SEQUENCIA_SEQ
...
alter table H_SEQUENCIA add constraint FK_H_SEQUENCIA_01
  foreign key (ATRIBUTO_ID) references ATRIBUTO(ATRIBUTOID);

-------------------------------------------------------------------------------
-- Alterações em dados básicos do TraceGP
-------------------------------------------------------------------------------

update aba 
   set src = 'CalendarioProjeto.do?command=abaCalendarioProjetoAction'
             ||chr(38)|| 'projeto_quadro_avisos=<form.DadosProjetoForm.idProjeto/>' 
where src like 'Calendario.do?command=abaCalendarioProjetoAction%';
commit;
/

insert into regras_tipo_propriedade (id, titulo, tipo_valor_id, vigente, where_join, 
                                     tipo_entidade_id, ref_tipo_entidade_id, chave, atualizavel)
       values (987, 'Árvore de Custos (Atividades e Tarefas)', 7, 'Y', 
              '([ENTIDADE-FILHA].TIPO_ENTIDADE,[ENTIDADE-FILHA].ENTIDADE_ID) in  (select ''P'', [ENTIDADE-PAI].ID from dual ' || 
              'union all select ''A'', id from atividade where projeto = [ENTIDADE-PAI].ID ' ||
              'union all select ''T'', id from tarefa where projeto = [ENTIDADE-PAI].ID)',
              2, 12, 'N', 'Y');
commit;
/


-- Mais de um registro por update

update regras_tipo_propriedade
   set atualizavel = 'Y'
 where titulo in ('label.prompt.responsavel', 'label.prompt.solicitante','label.prompt.atributo')
   and tipo_entidade_id = 1;
   
update regras_tipo_propriedade
   set atualizavel = 'Y'
 where titulo in ('label.prompt.atributo', 'label.prompt.arvoreCustos','label.prompt.titulo', 'Árvore de Custos (Atividades e Tarefas)','label.prompt.gerente') 
   and tipo_entidade_id = 2;
   
update regras_tipo_propriedade
   set atualizavel = 'Y'
 where titulo in ('label.prompt.lancamentos') 
   and tipo_entidade_id = 12;
...
-------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION GET_SITUACOES_ATRASADAS(situacao number, dataInicio date, dataFim date) RETURN NUMBER IS
  ret number;
BEGIN
  if (situacao = 1 and dataInicio < trunc(sysdate)) then
    ret := -2;
  elsif (situacao = 2 and dataFim < trunc(sysdate)) then
    ret := -3;
  else
    ret := situacao;
  end if;
...
values((select max(detalhe_disponivel_info_id)+1 from detalhe_disponivel_info), 
	 'label.prompt.dataInicioBaseline',
	 8, 'S', 'dataInicioBaseline', 
	 (select max(ordem) + 1 from detalhe_disponivel_info where informacao_disponivel_doc_id = 8 ));
	 
--Fim previsto baseline       
insert into detalhe_disponivel_info(detalhe_disponivel_info_id, titulo, informacao_disponivel_doc_id, vigente, codigo, ordem)
values((select max(detalhe_disponivel_info_id)+1 from detalhe_disponivel_info), 
	 'label.prompt.dataFimBaseline',
	 8, 'S', 'dataFimBaseline', 
	 (select max(ordem) + 1 from detalhe_disponivel_info where informacao_disponivel_doc_id = 8 ));

--Hora prevista baseline       
insert into detalhe_disponivel_info(detalhe_disponivel_info_id, titulo, informacao_disponivel_doc_id, vigente, codigo, ordem)
values((select max(detalhe_disponivel_info_id)+1 from detalhe_disponivel_info), 
	 'label.prompt.horasPrevistasBaseline',
	 8, 'S', 'horasPrevistasBaseline', 
	 (select max(ordem) + 1 from detalhe_disponivel_info where informacao_disponivel_doc_id = 8 ));

--Hora Realizada baseline       
insert into detalhe_disponivel_info(detalhe_disponivel_info_id, titulo, informacao_disponivel_doc_id, vigente, codigo, ordem)
values((select max(detalhe_disponivel_info_id)+1 from detalhe_disponivel_info), 
	 'label.prompt.horasRealizadasBaseline',
	 8, 'S', 'horasRealizadasBaseline', 
	 (select max(ordem) + 1 from detalhe_disponivel_info where informacao_disponivel_doc_id = 8 ));     

-- PCK_CONDICIONAL - demanda 13672
  ...
  for contador2 in 1..idDemandas.count loop
    for dom in (select de.demanda_id, te.obrigar_documento, te.obrigar_ter_documento, te.obrigar_entidade, te.obrigar_ter_entidade from transicao_estado te, demanda de
             ...
             if dom.obrigar_entidade = 'S' then
               ...
               select count(*) into contador from demanda_entidade_estado de
               where de.demanda_id = dom.demanda_id;
...
--John - Demanda 13720
--Recompilar a view v_custo_hora_trabalhada com a nova correção

create or replace view v_custo_hora_trabalhada as
select x.DATA_TRAB, x.USUARIO_ID, x.VALOR_HORA, x.MINUTOS_TRAB,
       x.TAREFA, (x.MINUTOS_TRAB / 60) * x.VALOR_HORA CUSTO_TOTAL
  from (select ht.datatrabalho DATA_TRAB, ht.responsavel USUARIO_ID,
               nvl((select max(vvhs.valor_hora)
                      from mv_valor_hora_simplificada vvhs
                     where vvhs.usuario_id          = ht.responsavel
                       and nvl(vvhs.tp_vigente,'S') = 'S'
                       and vvhs.data_inicio        <= ht.datatrabalho
                       and vvhs.data_inicio         = (select max(vvhs2.data_inicio)
                                                         from mv_valor_hora_simplificada vvhs2
                                                        where vvhs2.usuario_id           = ht.responsavel
                                                          and vvhs2.data_inicio         <= ht.datatrabalho
                                                          and nvl(vvhs2.tp_vigente, 'S') = 'S')),0) VALOR_HORA,
               ht.minutos MINUTOS_TRAB, ht.tarefa TAREFA
          from horatrabalhada ht) x;

--incluir o delete abaixo para que os registros duplicados sejam excluídos
delete from  valor_hora_usuario 
where valor_hora_usuario.valor_hora_usuario_id = 
(select max(valor_hora_usuario.valor_hora_usuario_id) from valor_hora_usuario 
where 1 <(select count(v.valor_hora_usuario_id) 
                                     from valor_hora_usuario v
                                     where v.usuario_id = valor_hora_usuario.usuario_id
                                     and v.data_inicio_valor_hora = valor_hora_usuario.data_inicio_valor_hora))
;
...
insert into versao(id, titulo, patch, versao_tgp_id, comentario)
       values (versao_seq.nextval, '6.0.1', '01', 4, 'Aplicação de Patch');
commit;
/
