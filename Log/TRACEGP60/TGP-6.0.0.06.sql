/*****************************************************************************\
* TraceGP patch 6 (6.0.0)                                                     *
\*****************************************************************************/
--------------------
alter table baseline_custo_lancamento drop constraint CHK_BASELINE_CUSTO_LANC_01;
alter table baseline_custo_lancamento add constraint CHK_BASELINE_CUSTO_LANC_01
  check (tipo in ('P', 'R', 'O'));

--------------------
alter table projeto add baseline_comparacao number(10);
ALTER TABLE PROJETO ADD CONSTRAINT FK_PROJETO_09 
  FOREIGN KEY (BASELINE_COMPARACAO) REFERENCES BASELINE(BASELINE_ID)
ON DELETE cascade;

declare
  ln_prox number;
begin
  select nvl(max(id),0) + 1 into ln_prox
    from relat_relatorio;   
  execute immediate 'create sequence relat_relatorio_seq increment by 1 start with ' || 
                    ln_prox || ' maxvalue 9999999999 minvalue 1 nocache';                   
end;
/

insert into versao_sequencia (id, versao_tgp_id, nome_sequencia, tabela, coluna) 
       values (versao_sequencia_seq.nextval, 3, 'relat_relatorio_seq', 'relat_relatorio_seq', 'id');
commit;
/

truncate table dia;

insert into dia(data) 
select add_months(trunc(sysdate),360)-level+1 dia
  from dual 
connect by add_months(trunc(sysdate),360)-level+1 >= to_date('20000101','yyyymmdd');
...
declare
  ln_prox number;
begin
  select max(nvl(id,0)) + 1 into ln_prox
    from regras_tipo_propriedade;   
  execute immediate 'create sequence regras_tipo_propriedade_seq increment by 1 start with ' || 
                    ln_prox || ' maxvalue 9999999999 minvalue 1 nocache';                   
end;
/

insert into versao_sequencia (id, versao_tgp_id, nome_sequencia, tabela, coluna) 
       values (versao_sequencia_seq.nextval, 3, 'regras_tipo_propriedade_seq', 'regras_tipo_propriedade', 'id');
commit;
/
...
begin
  select count(1) into ln_conta 
    from regras_tipo_propriedade
   where tipo_entidade_id     = 5
     and ref_tipo_entidade_id = 10;
  
  if ln_conta = 0 then
    select regras_tipo_propriedade_seq.nextval into ln_seq from dual;
    insert into regras_tipo_propriedade(id, titulo, coluna, tipo_valor_id, vigente, tipo_entidade_id, ref_tipo_entidade_id, chave)
           values(ln_seq, 'Dominios', null, 6, 'N',5, 10, 'N');
  end if;
  commit;
end;
/

-- Cria telas relatório dinãmico em projetos
declare 
  ln_conta number;
  ln_tela  number;
  ln_doc   number;
...
begin
  select count(1) into ln_conta
    from tela
   where codigo = 'RELAT_RELATORIO_PROJETO';
  ...
  if ln_conta = 0 then
    ...
    insert into tela (TELAID, NOME, URL, VISIVEL, GRUPOID, ORDEM, CODIGO, SUBGRUPO, ATALHO)
           values (ln_tela, 'bd.tela.relatorioDinamico', 'RelatRelatorioProjeto.do?command=defaultAction', 
                   'S', 16, 99, 'RELAT_RELATORIO_PROJETO', 'QUARTO', 'N');
                   
    select nvl(max(informacao_disponivel_doc_id),0)+1 into ln_doc from informacao_disponivel_doc; 
    insert into informacao_disponivel_doc (INFORMACAO_DISPONIVEL_DOC_ID, TITULO, TELAID, POSSUI_HIERARQUIA, VIGENTE, CODIGO)
           values (ln_doc, 'bd.tela.relatorioDinamico', ln_tela, 'N', 'S', 'RELAT_RELATORIO_PROJETO');

    select nvl(max(detalhe_disponivel_info_id),0)+1 into ln_info from detalhe_disponivel_info;
    insert into detalhe_disponivel_info (DETALHE_DISPONIVEL_INFO_ID, TITULO, INFORMACAO_DISPONIVEL_DOC_ID, VIGENTE, CODIGO, ORDEM)
           values (ln_info, 'bd.tela.relatorioDinamico', ln_doc, 'S', 'RELAT_RELATORIO_PROJETO', 3);
  end if;
  commit;
end;
/
...
update regras_tipo_propriedade
   set where_join = '[ENTIDADE-PAI].ID = [ENTIDADE-FILHA].IDENTIDADE   AND [ENTIDADE-FILHA].TIPOENTIDADE   = ''E''',
       tipo_valor_id = 5
 where id = 661;
update regras_tipo_propriedade
...
/*Alerta quando copiar > 4000 char*/'      when dbms_lob.getlength([ALIAS-TAB-ATRIB].valor_html) > 4000 then to_char(to_number(''campo html grande demais. Gera erro.''))' ||
   ...
   begin
      ...
      if pn_propriedade_id_destino is not null then
         ...
         lv_sql_destino := f_get_val_sel_propriedade(pn_demanda_id, pv_usuario_id, pn_propriedade_id_origem, true, ln_tipo_entidade_id);
      end if;
...
insert into versao(id, titulo, patch, versao_tgp_id, comentario)
       values (versao_seq.nextval, '6.0.0', '06', 3, 'Aplicação de patch');
commit;
/
