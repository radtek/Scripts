...
ALTER TABLE ATRIBUTO MODIFY TIPO VARCHAR2(3);
ALTER TABLE ATRIBUTO ADD ENTIDADE_MATRIZ_ID NUMBER(10);

ALTER TABLE ATRIBUTO_COLUNA ADD CHAVE VARCHAR2(1) DEFAULT 'N';

ALTER TABLE CONFIGURACOES
ADD VISU_DESC_ATRIBUTOS VARCHAR2(1 BYTE);

ALTER TABLE relat_dataset_campo
ADD ALINHAMENTO VARCHAR2(6 BYTE) DEFAULT 'left';
...
alter table ACAO_CONDICIONAL add constraint CHK_ACAO_CONDICIONAL_01
  check (acao in ('DE','EX','HA','LI','OC','PO','OB','TO', 'PF', 'DS', 'EE', 'GM', 'GB', 'AM', 'VE', 'CO', 'CL', 'CP', 'GD', 'EL'));

alter table COMP_MODELO_IMPRESSAO modify VALOR VARCHAR2(4000);
-- Criação de novos objetos

CREATE TABLE MATRIZ_ATRIBUTO_ENTIDADE ( 
  MATRIZ_ATRIBUTO_ID NUMBER(10)  NOT NULL,
  ENTIDADE_ID        NUMBER(10)  NOT NULL, 
  TIPO_ENTIDADE      VARCHAR2(1) NOT null,
constraint PK_MATRIZ_ATRIBUTO_ENTIDADE primary key (MATRIZ_ATRIBUTO_ID, ENTIDADE_ID, TIPO_ENTIDADE) using index tablespace &CS_TBL_IND 
...
CREATE TABLE ATRIBUTO_PERFIL ( 
  ...
  INCLUSAO_EDICAO  VARCHAR2(1) DEFAULT 'Y',
  SOMENTE_LEITURA  VARCHAR2(1) DEFAULT 'Y',
constraint PK_ATRIBUTO_PERFIL primary key (id) using index tablespace &CS_TBL_IND 
) tablespace &CS_TBL_DAT;

CREATE SEQUENCE ATRIBUTO_PERFIL_SEQ  START WITH 1 INCREMENT BY 1 NOCACHE;

CREATE TABLE ATRIBUTO_COLUNA_FORMULARIO ( 
  ID                      NUMBER(10,0)     NOT NULL ENABLE,
  ATRIBUTO_PRINCIPAL_ID   NUMBER(10,0)     NOT NULL, 
  ATRIBUTO_RELACIONADO_ID NUMBER(10,0)     NOT NULL, 
  FORMULARIO_ID       NUMBER(10,0)     NOT NULL,
  VISIVEL             VARCHAR2(1 BYTE) DEFAULT 'Y',
  TAMANHO             NUMBER(10,0),
constraint PK_ATRIBUTO_COLUNA_FORMULARIO primary key (id) using index tablespace &CS_TBL_IND 
) tablespace &CS_TBL_DAT;

CREATE SEQUENCE ATRIBUTO_COLUNA_FORMULARIO_SEQ  START WITH 1 INCREMENT BY 1 NOCACHE;
...
ALTER TABLE ATRIBUTO ADD CONSTRAINT FK_ATRIBUTO_02 
  FOREIGN KEY (ENTIDADE_MATRIZ_ID) REFERENCES MATRIZ (ID) ON DELETE CASCADE;
ALTER TABLE ATRIBUTO_COLUNA_FORMULARIO ADD CONSTRAINT FK_ATRIBUTO_COLUNA_FORM_01 
  FOREIGN KEY (ATRIBUTO_PRINCIPAL_ID) REFERENCES ATRIBUTO (ATRIBUTOID) ON DELETE CASCADE;
ALTER TABLE ATRIBUTO_COLUNA_FORMULARIO ADD CONSTRAINT FK_ATRIBUTO_COLUNA_FORM_02 
  FOREIGN KEY (ATRIBUTO_RELACIONADO_ID) REFERENCES ATRIBUTO (ATRIBUTOID) ON DELETE CASCADE;
ALTER TABLE ATRIBUTO_COLUNA_FORMULARIO ADD CONSTRAINT FK_ATRIBUTO_COLUNA_FORM_03 
  FOREIGN KEY (FORMULARIO_ID) REFERENCES FORMULARIO (FORMULARIO_ID) ON DELETE CASCADE;
ALTER TABLE ATRIBUTO_PERFIL ADD CONSTRAINT FK_ATRIBUTO_PERFIL_01 
  FOREIGN KEY (ATRIBUTO_ID) REFERENCES ATRIBUTO (ATRIBUTOID) ON DELETE CASCADE;
ALTER TABLE ATRIBUTO_PERFIL ADD CONSTRAINT FK_ATRIBUTO_PERFIL_02 
  FOREIGN KEY (PERFIL_ID) REFERENCES PERFIL (PERFILID) ON DELETE CASCADE;
alter table DASHBOARD_PERFIL add constraint FK_DASHBOARD_PERFIL_01 
  foreign key (DASHBOARD_ID) references DASHBOARD (ID) on delete cascade;
alter table DASHBOARD_PERFIL add constraint FK_DASHBOARD_PERFIL_02 
  foreign key (PERFIL_ID) references PERFIL (PERFILID) on delete cascade;
ALTER TABLE MATRIZ_ATRIBUTO_ENTIDADE ADD CONSTRAINT FK_MATRIZ_ATRIBUTO_ENTIDADE_01 
...
SELECT x.id PROJETO_ID,
    x.data,
    CASE WHEN X.DATA >= (SELECT PROJ.PRAZOPREVISTO FROM PROJETO PROJ WHERE PROJ.ID = X.ID) THEN
         (SELECT PROJ.Horasprevistas FROM PROJETO PROJ WHERE PROJ.ID = X.ID)
    ELSE
         SUM(NVL(minutos_ate_data,0))
    END PREVISTO,
  ...
  FROM
    ...
    FROM v_dias_futuro vdf,
      tarefa t,
      projeto p
    WHERE vdf.dia BETWEEN least(NVL(p.iniciorealizado, to_date('31129999', 'ddmmyyyy')), 
    	NVL(p.datainicio, to_date('31129999', 'ddmmyyyy'))) AND greatest(NVL(p.prazorealizado, to_date('01011900', 'ddmmyyyy')), 
    		NVL(p.prazoprevisto, to_date('01011900', 'ddmmyyyy')))
    AND t.projeto = p.id
    UNION ALL
    -- REALIZADO
    SELECT p.id,
    ...
    FROM v_dias_futuro vdf,
      ...
      tarefa t,
      projeto p
    WHERE vdf.dia between (select min(datatrabalho) from horatrabalhada
                               where tarefa in (select id from tarefa where projeto = p.id))
                      and    (select max(datatrabalho) from horatrabalhada
                               where tarefa in (select id from tarefa where projeto = p.id))
    AND ht.tarefa        = t.id
    AND t.projeto        = p.id
    UNION ALL
...
create or replace function f_perc_usado_previsto (h_realizada number, h_previsto number) return number is
  ln_ret  number;

begin
    if h_realizada <= 0 or h_previsto <= 0 then
      return 0;
    else
      ln_ret := (h_realizada / h_previsto) * 100; 
      if ln_ret > 100 then
        return 100;
      else
        return ln_ret;
...
declare
  ln_id_projeto number;
  lb_todas_concluidas boolean;
begin 
  --ln_id_projeto := 9;
  for ln_id_projeto in (select id from projeto) loop
  --
  lb_todas_concluidas := false;
  --Começo a persquisa de atividades de traz para frente, fazendo com que seja processado da filha para a pai
  for atv_ in (select * from atividade where atividade.projeto = ln_id_projeto.id and atividade.situacao <> 3 order by id desc) loop
       dbms_output.put_line('atv:' || atv_.id);       
       --Verifico se todas as tarefas estão concluídas
       for tar_ in (select * from tarefa where tarefa.atividade = atv_.id and tarefa.projeto = ln_id_projeto.id) loop
           if tar_.situacao = 3 then
             lb_todas_concluidas := true;
           else 
             lb_todas_concluidas := false;  
           end if;  
           dbms_output.put_line('TAR lb_todas_concluidas[' || to_char(tar_.id) || '],[' || tar_.situacao || ']:' || ((CASE lb_todas_concluidas when true then 'Y' ELSE 'N' END)));
           EXIT WHEN lb_todas_concluidas = false;
       end loop;
       --Verifico se todas as atividades filhas estão concluídas
       for atv_filha_ in (select * from atividade where atividade.projeto = ln_id_projeto.id 
                          and atividade.tipoentidadepai = 'A' and atividade.entidadepai = atv_.id) loop
         if atv_filha_.situacao = 3 then
             lb_todas_concluidas := true;           
         else 
             lb_todas_concluidas := false;           
         end if;
         dbms_output.put_line('ATV FILHA - lb_todas_concluidas[' || to_char(atv_filha_.id) || '],[' || atv_filha_.situacao || ']:' || ((CASE lb_todas_concluidas when true then 'Y' ELSE 'N' END)));
         EXIT WHEN lb_todas_concluidas = false;
       end loop;
       
       --Se todas as tarefas concluídas deve concluir a atividade
       if lb_todas_concluidas = true then
         dbms_output.put_line('-------Alterando a atividade ['|| atv_.id ||']...... ');
         update atividade set atividade.situacao = 3, atividade.porcentagemconcluida = 100 where atividade.id = atv_.id;
       end if;
  end loop;  
  -- 
  end loop;
...
-- Associa dashboards públicos a todos perfis
declare
  ln_conta number;
begin
  for c_perfil in (select perfilid from perfil)loop
    for c_dashboard in (select id from dashboard d where d.publico = 'Y') loop
      select count(1) into ln_conta
        from dashboard_perfil
       where dashboard_id = c_dashboard.id
         and perfil_id    = c_perfil.perfilid;
      if ln_conta = 0 then
        insert into dashboard_perfil (dashboard_id,perfil_id)
               values (c_dashboard.id, c_perfil.perfilid);
      end if;
    end loop;
  end loop;
end;
...
insert into versao(id, titulo, patch, versao_tgp_id, comentario)
       values (versao_seq.nextval, '6.0.1', '03', 4, 'Aplicação de Patch');
commit;
/
