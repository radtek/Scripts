...
--
ALTER TABLE BASELINE_ENTIDADE ADD CPI_MONETARIO NUMBER(21,2);
ALTER TABLE BASELINE_ENTIDADE ADD SPI_MONETARIO NUMBER(21,2);
ALTER TABLE BASELINE_ENTIDADE ADD PERC_CONCLUIDO NUMBER(5,2);

alter table log_hist_transicao add tipo_dado varchar(1);
 
alter table REGRAS_TIPO_PROPRIEDADE add ATUALIZAVEL varchar2(1) default 'N' not null;
comment on column REGRAS_TIPO_PROPRIEDADE.ATUALIZAVEL
  is 'Pode ser atualizado pela copia de propriedades';
  
alter table RELAT_DATASET_CAMPO modify SIGLA VARCHAR2(32);
  
-- ATUALIZACOES

update regras_tipo_propriedade set vigente  = 'Y' where tipo_entidade_id = 2 and titulo = 'ID';
commit;
/
...
begin
  select count(1)
    into ln_conta
    from user_objects
   where object_name = 'VW_ZEUS';
   
  if ln_conta = 0 then
    execute immediate 'CREATE TABLE VW_ZEUS (DATA_LANCTO DATE, ANO VARCHAR2(30), MES VARCHAR2(30),' ||
                             'EMPRESA_COD VARCHAR2(4000), UNIDADE_COD VARCHAR2(10), CR_COD VARCHAR2(16),' ||
                             'CONTA_COD VARCHAR2(16), CONTA_COD_CTB VARCHAR2(16), VALOR NUMBER(16,2),' ||
                             'DESCRICAO VARCHAR2(255), DEB_CRED CHAR(1), DATA_FECHTO DATE)';
  end if;
end;
/ 
...
insert into versao(id, titulo, patch, versao_tgp_id, comentario)
       values (versao_seq.nextval, '6.0.0', '08', 3, 'Aplicação de patch');
commit;
/
