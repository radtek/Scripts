...
DROP TABLE LOG_LISTA_HIST_TRANS;

CREATE TABLE LOG_LISTA_HIST_TRANS ( 
  ID           number(10) not null,
  LISTA_ID     number(10) not null,
  LOG_PAI_ID   NUMBER(10) not null,
  ITEM         number(10),
  VALOR        varchar2(150),
  VALOR_TITULO varchar2(150),
...
ALTER TABLE LOG_LISTA_HIST_TRANS ADD CONSTRAINT FK_LOG_LISTA_HIST_TRANS_01 
  FOREIGN KEY (LOG_PAI_ID) REFERENCES LOG_HIST_TRANSICAO(ID) ON DELETE cascade;

alter table LOG_HIST_TRANSICAO modify resultado varchar2(1000);
...
begin
  ...
  if ln_conta = 0 then
    insert into tracegp_config (variavel, valor_varchar, comentario) 
           values ('PROC_NOTURNO: APUR_INDICADORES', 'Y', 
             'Marca indicando que o processo noturno do TraceGP deve executar a apuração de indicadores');
    commit;
  end if;
  ...
  if ln_conta = 0 then
    insert into tracegp_config (variavel, valor_varchar, comentario) 
           values ('PROC_NOTURNO: COPIA_SPI_CPI', 'Y', 
             'Marca indicando que o processo noturno do TraceGP deve executar cópia completa de SPI e CPI');
    commit;
  end if;
end;
/
...
create or replace package pck_regras is
  ...
  function f_atr_obrig_nao_preenchido(pn_id_entidade number, pn_id_lancamento number) return number;
...
/*Alerta quando copiar > 4000 char*/'      when dbms_lob.getlength([ALIAS-TAB-ATRIB].valor_html) > 4000 then to_char(to_number(''campo html grande demais. Gera erro.''))' ||
                ...
                if ln_linha = ln_total then
                   ...
                   while true loop
                      ...
                      while true loop
                         ...
                         ln_atr_obrig := 0;
                         select tipo_entidade into lv_tipo_ent_cust 
                         from custo_entidade where custo_entidade.id = rec_lancamento.custo_entidade_id;
                         if lv_tipo_ent_cust = 'D' then--Demanda
                                ln_atr_obrig := f_atr_obrig_nao_preenchido(ln_entidade, rec_lancamento.id);
   ...
   function f_atr_obrig_nao_preenchido(pn_id_entidade number, pn_id_lancamento number) return number is
     ln_count number;
   begin
        ln_count := 0;      
   ...
   procedure get_v_padrao_atr_lanc(pn_id_entidade number, 
                                   ...
                                   pn_dominio out number,
                                   pv_valorhtml out clob,
                                   pn_categoria out number) is
     begin
          
            select nvl(CONFIGFORMULARIO.valor, CONFIGPADRAO.valor) valor,
             nvl(CONFIGFORMULARIO.valordata, CONFIGPADRAO.valordata) valordata,
             nvl(CONFIGFORMULARIO.valornumerico, CONFIGPADRAO.valornumerico) valornumerico,
             nvl(CONFIGFORMULARIO.dominio_atributo_id, CONFIGPADRAO.dominio_atributo_id) dominio,
             nvl(CONFIGFORMULARIO.valor_html, CONFIGPADRAO.valor_html) valorhtml,
             nvl(CONFIGFORMULARIO.categoria_item_atributo_id, CONFIGPADRAO.categoria_item_atributo_id) categoria
--             nvl(CONFIGFORMULARIO.obrigatorio, CONFIGPADRAO.obrigatorio) obrigatorio
            ...
            from ( select ael.tipo_lancamento_id,ael.identidade,ael.tipoentidade,ael.atributoid,ael.obrigatorio, aev.valor,
                   ...
                   where ael.tipo_lancamento_id  = aev.tipo_lancamento_id(+)
                     ...
                     and ael.tipoentidade       = aev.tipoentidade(+)
                     and ael.atributoid         = aev.atributoid(+)
                     and ael.tipoentidade       = 'D') CONFIGFORMULARIO,
                 ( select ael.tipo_lancamento_id,ael.identidade,ael.tipoentidade,ael.atributoid,ael.obrigatorio, aev.valor,
                          aev.valordata, aev.valornumerico, aev.dominio_atributo_id, aev.valor_html, aev.categoria_item_atributo_id        
                   from atributoentidade_lancamento  ael,
                        atributoentidade_valorpadrao aev
...
delete from categoria_conhec_form 
 where categoria_conh_id in (select id from categoria_conhecimento where projeto_id is not null);
commit;
/
...
insert into versao(id, titulo, patch, versao_tgp_id, comentario)
       values (versao_seq.nextval, '6.0.0', '05', 3, 'Aplicação de patch');
commit;
/
