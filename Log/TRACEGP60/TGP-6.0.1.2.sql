...
create or replace view v_objetivos_indicadores_resumo as
	select mi.id id, 'I' TIPO, mm.data_limite, indicador_template ,mi.entidade_id ENTIDADE_ID, mi.tipo_entidade TIPO_ENTIDADE,
	...
	union all
	select mo.id id, 'O' TIPO, mom.data_limite, 'N' indicador_template, mo.entidade_id ENTIDADE_ID, mo.tipo_entidade TIPO_ENTIDADE,
...
-- FIM v_objetivos_indicadores_resumo;

-- TAG DE AGRUPAMENTO
insert into tela (TELAID, nome, url, visivel, grupoid, ordem, codigo, subgrupo, atalho)
values  ((select max(telaid) + 1 from tela),	'bd.tela.tiposAgrupamentos', null,	'S', 7, null,	'TIPOS_AGRUPAMENTO_CUSTOS',	null, 'N');
...
CREATE TABLE TIPO_AGRUPAMENTO (
    ...
    constraint CHK_TIPO_AGRUPAMENTO_01 check (VIGENTE in ('Y', 'N'))
  );
  
ALTER TABLE TIPO_AGRUPAMENTO ADD CONSTRAINT PK_TIPO_AGRUPAMENTO_CUSTO PRIMARY KEY (ID);
CREATE SEQUENCE TIPO_AGRUPAMENTO_SEQ START WITH 1 INCREMENT BY 1 NOCACHE;

-------------------------------------------------------------------------------------------------------------------------------------------------------

CREATE TABLE TIPO_AGRUPAMENTO_ENTIDADE ( 
  ID NUMBER(10) NOT NULL,
  IDENTIDADE NUMBER(10,0), 
  TIPOENTIDADE       VARCHAR2(1 BYTE),
  TIPO_AGRUPAMENTO_ID NUMBER(10) NOT NULL
);


ALTER TABLE TIPO_AGRUPAMENTO_ENTIDADE ADD CONSTRAINT PK_TIPO_AGRUPAMENTO_ENTIDADE PRIMARY KEY (ID);
ALTER TABLE TIPO_AGRUPAMENTO_ENTIDADE ADD CONSTRAINT FK_TP_AGRUPAMENTO_ENTIDADE_01 FOREIGN KEY (TIPO_AGRUPAMENTO_ID) REFERENCES TIPO_AGRUPAMENTO (ID) ON DELETE CASCADE;

CREATE SEQUENCE TIPO_AGRUPAMENTO_ENTIDADE_SEQ  START WITH 1 INCREMENT BY 1 NOCACHE;


-------------------------------------------------------------------------------------------------------------------------------------------------------

CREATE TABLE CUSTO_RECEITA_AGRUPAMENTO (
    TIPO_AGRUPAMENTO_ID NUMBER(10,0) NOT NULL ENABLE, 
    CUSTO_RECEITA_ID    NUMBER(10,0) NOT NULL ENABLE
);   

ALTER TABLE CUSTO_RECEITA_AGRUPAMENTO ADD CONSTRAINT PK_CUSTO_RECEITA_AGRUPAMENTO PRIMARY KEY (TIPO_AGRUPAMENTO_ID, CUSTO_RECEITA_ID);
ALTER TABLE CUSTO_RECEITA_AGRUPAMENTO ADD CONSTRAINT FK_CUSTO_RECEITA_AGRU_01 FOREIGN KEY (TIPO_AGRUPAMENTO_ID) REFERENCES TIPO_AGRUPAMENTO (ID) ON DELETE CASCADE ENABLE;
ALTER TABLE CUSTO_RECEITA_AGRUPAMENTO ADD CONSTRAINT FK_CUSTO_RECEITA_AGRU_02 FOREIGN KEY (CUSTO_RECEITA_ID) REFERENCES CUSTO_RECEITA (ID) ON DELETE CASCADE ENABLE;

-------------------------------------------------------------------------------------------------------------------------------------------------------


create or replace view V_CUSTO_RECEITA as
	select saida.id, 
		   saida.titulo, 
		   saida.tipo, 
		   saida.VIGENTE, 
		   saida.ID_PAI, 
		   saida.DATA_INICIO_LANCAMENTO, 
		   saida.DATA_FIM_LANCAMENTO, 
		   saida.DATA_INICIO_VISUALIZACAO, 
		   saida.DATA_FIM_VISUALIZACAO,
		   filtro.id as FILTRO_ID, 
		   filtro.titulo as FILTRO_TITULO, 
		   filtro.tipo as FILTRO_TIPO, 
		   filtro.VIGENTE as FILTRO_VIGENTE, 
		   filtro.ID_PAI as FILTRO_ID_PAI, 
		   filtro.DATA_INICIO_LANCAMENTO as FILTRO_DT_INI_LANCAMENTO, 
		   filtro.DATA_FIM_LANCAMENTO as FILTRO_DT_FIM_LANCAMENTO, 
		   filtro.DATA_INICIO_VISUALIZACAO as FILTRO_DT_INI_VISUALIZACAO,  
		   filtro.DATA_FIM_VISUALIZACAO as FILTRO_DT_FIM_VISUALIZACAO
	  from custo_receita saida,
		   custo_receita filtro,
		   (select to_number(trim(SUBSTR(path, 2, 11))) folha, id
			  from (SELECT sys_connect_by_path(TO_CHAR(cr.id,'9999999999'), '-') path, cr.id
					  FROM custo_receita cr
					CONNECT BY PRIOR cr.id_pai = cr.id
					start with cr.id in (select id from custo_receita where id not in (select distinct nvl(id_pai,0) from custo_Receita)))) tab
	 where filtro.id = tab.folha
	   and saida.id = tab.id;


-------------------------------------------------------------------------------------------------------------------------------------------------------

CREATE TABLE TIPO_LANCAMENTO_AGRUPAMENTO (
    TIPO_AGRUPAMENTO_ID NUMBER(10,0) NOT NULL ENABLE, 
    TIPO_LANCAMENTO_ENTIDADE_ID  NUMBER(10,0) NOT NULL ENABLE
);   

ALTER TABLE TIPO_LANCAMENTO_AGRUPAMENTO ADD CONSTRAINT PK_TIPO_LANCAMENTO_AGRUPAMENTO PRIMARY KEY (TIPO_AGRUPAMENTO_ID, TIPO_LANCAMENTO_ENTIDADE_ID);
ALTER TABLE TIPO_LANCAMENTO_AGRUPAMENTO ADD CONSTRAINT FK_TIPO_LANCAMENTO_AGRUP_01 FOREIGN KEY (TIPO_AGRUPAMENTO_ID) REFERENCES TIPO_AGRUPAMENTO (ID) ON DELETE CASCADE ENABLE;
ALTER TABLE TIPO_LANCAMENTO_AGRUPAMENTO ADD CONSTRAINT FK_TIPO_LANCAMENTO_AGRUP_02 FOREIGN KEY (TIPO_LANCAMENTO_ENTIDADE_ID) REFERENCES TIPO_LANCAMENTO_ENTIDADE (ID) ON DELETE CASCADE ENABLE;


-------------------------------------------------------------------------------------------------------------------------------------------------------
-- FIM TAG DE AGRUPAMENTO

--John
begin 
  --Data Ocorrência
  insert into detalhe_disponivel_info(detalhe_disponivel_info_id, titulo, informacao_disponivel_doc_id, vigente, codigo, ordem)
  values((select max(detalhe_disponivel_info_id)+1 from detalhe_disponivel_info), 
     'label.prompt.dataDaOcorrencia',
     12, 'S', 'DATA_OCORRENCIA', 
     (select max(ordem) + 1 from detalhe_disponivel_info where informacao_disponivel_doc_id = 12 ));
     
  ---Usuário Ocorrência   
  insert into detalhe_disponivel_info(detalhe_disponivel_info_id, titulo, informacao_disponivel_doc_id, vigente, codigo, ordem)
  values((select max(detalhe_disponivel_info_id)+1 from detalhe_disponivel_info), 
     'label.prompt.usuarioOCorrencia',
     12, 'S', 'USUARIO_OCORRENCIA', 
     (select max(ordem) + 1 from detalhe_disponivel_info where informacao_disponivel_doc_id = 12 ));

  --Assunto Ocorrência
  insert into detalhe_disponivel_info(detalhe_disponivel_info_id, titulo, informacao_disponivel_doc_id, vigente, codigo, ordem)
  values((select max(detalhe_disponivel_info_id)+1 from detalhe_disponivel_info), 
     'label.prompt.assuntoOcorrencia',
     12, 'S', 'ASSUNTO_OCORRENCIA', 
     (select max(ordem) + 1 from detalhe_disponivel_info where informacao_disponivel_doc_id = 12 ));

  --Tipo Ocorrência
  insert into detalhe_disponivel_info(detalhe_disponivel_info_id, titulo, informacao_disponivel_doc_id, vigente, codigo, ordem)
  values((select max(detalhe_disponivel_info_id)+1 from detalhe_disponivel_info), 
     'label.prompt.tipoDaOcorrencia',
     12, 'S', 'TIPO_OCORRENCIA', 
     (select max(ordem) + 1 from detalhe_disponivel_info where informacao_disponivel_doc_id = 12 ));
...
create or replace view v_custo_tarefa as
	...
	union
	select  /*+ ordered */
		   t.id, null as responsavel, nvl(pp.valor_hora,0) VALOR_HORA,
		   t.datainicio INICIO_VALOR,
		   t.prazoprevisto FINAL_VALOR,
			 t.datainicio INICIO_TAREFA, t.prazoprevisto FINAL_TAREFA,
			 (CASE WHEN (t.prazoprevisto - t.datainicio + 1) > 0 THEN nvl((t.horasprevistas/60) / (t.prazoprevisto - t.datainicio + 1),0) ELSE 0 END) HORAS_POR_DIA,
			 t.projeto PROJETO_ID
	from papelprojeto        pp,
		 tarefa              t
	where t.papelprojeto_id = pp.papelprojetoid (+)
	and t.projeto = pp.projetoid;

--Peruzzo


ALTER TABLE COMP_MODELO_IMPRESSAO ADD RELAT_RELATORIO_ID NUMBER(10,0);
ALTER TABLE COMP_MODELO_IMPRESSAO ADD FILTRO_ID NUMBER(10,0);

ALTER TABLE COMP_MODELO_IMPRESSAO ADD CONSTRAINT FK_COMP_MODELO_IMPRESSAO_02 FOREIGN KEY (RELAT_RELATORIO_ID) REFERENCES RELAT_RELATORIO (ID) ON DELETE CASCADE ENABLE;
ALTER TABLE COMP_MODELO_IMPRESSAO ADD CONSTRAINT FK_COMP_MODELO_IMPRESSAO_03 FOREIGN KEY (FILTRO_ID) REFERENCES FILTRO (ID) ON DELETE CASCADE ENABLE;


ALTER TABLE COMP_MODELO_IMPRESSAO
DROP CONSTRAINT CHK_COMP_MODELO_IMPRESSAO_01;

ALTER TABLE COMP_MODELO_IMPRESSAO
ADD CONSTRAINT CHK_COMP_MODELO_IMPRESSAO_01
   CHECK (TIPO IN ('D', 'L', 'H','V','Z','I','R'));



insert into relat_componente (id, titulo, sigla) values ((select MAX(ID)+1 from relat_componente), 'label.prompt.demandaCorrente', 'demanda');
...
-------------------------------------------------------------------------------
-- Packages e functions
-------------------------------------------------------------------------------
create or replace function f_perc_previsto (pd_inicio date, pd_termino date) return number is
  ld_data date;
  ln_ret  number;

begin
  if pd_inicio <= trunc(sysdate) then
    ld_data := least(trunc(sysdate), pd_termino);
    ln_ret  := ( (ld_data - pd_inicio + 1) / (pd_termino - pd_inicio + 1) );
    if ln_ret <= 0 then
      return 0;
    else
      if ln_ret >= 1 then
...
function  f_lista_campos  return lista_campos PIPELINED
  as
  begin
         pipe row( t_tipo_campos( 1,'Codigo'   ,'char'  ,4 ,0,'' ,'X'                    ,'Y',''                    ,'Codigo do Sistema que gerou a informac?o Orcamento (ORC)') );
         pipe row( t_tipo_campos( 2,'tipo'     ,'number',2 ,0,'' ,'99'                   ,'Y','[2],[3],[4],[5],[6]' ,'Tipo de movimento. Este codigo e criado no sistema do orcamento e determina a informac?o, que estara trafegando o movimento. (2  Orcado,3  Realizado,4  Transposto,5  Suplementado,6  Retificado)'));
         pipe row( t_tipo_campos( 3,'ano'      ,'number',4 ,0,'' ,'9999'                 ,'Y',''                    ,'Ano do Orcamento'));
         pipe row( t_tipo_campos( 4,'mes'      ,'number',2 ,0,'' ,'99'                   ,'Y',''                    ,'Mes do Orcamento'));
         pipe row( t_tipo_campos( 5,'cod_empre','number',3 ,0,'' ,'999'                  ,'Y',''                    ,'Codigo da Empresa do Movimento'));
         pipe row( t_tipo_campos( 6,'Cod_UO'   ,'char'  ,10,0,'' ,'X '                   ,'Y',''                    ,'Codigo da Unidade Organizacional'));
         pipe row( t_tipo_campos( 7,'Cod_CR'   ,'char'  ,16,0,'' ,'X '                   ,'Y',''                    ,'Codigo do Centro de Responsabilidade'));
         pipe row( t_tipo_campos( 8,'Cod_CO'   ,'char'  ,16,0,'' ,'X '                   ,'Y',''                    ,'Codigo do Conta Orcamentaria'));
         ...
         pipe row( t_tipo_campos(12,'Nome_Arq' ,'char'  ,4 ,0,'' ,'X '                   ,'Y',''                    ,'Nome do arquivo do movimento orcamentario'));
         pipe row( t_tipo_campos(13,'Ano_PC'   ,'date'  ,4 ,0,'' ,'YYYY'                 ,'Y',''                    ,'Ano do Plano de Contas'));
         pipe row( t_tipo_campos(14,'Cod_CC'   ,'char'  ,16,0,'' ,'X '                   ,'Y',''                    ,'Codigo do Conta Contabil'));
         pipe row( t_tipo_campos(15,'Cod_Mov'  ,'char'  ,1 ,0,'' ,'X'                    ,'Y','[A],[M]'             ,'Determina a forma da entrada do movimento do orcamento. Todos os movimentos atuais s?o automaticos M  Manual A - Automatico'));
         pipe row( t_tipo_campos(16,'Dt_Mov'   ,'date'  ,19,0,'' ,'DD/MM/YYYY HH24:MI:SS','Y',''                    ,'Data de Atualizac?o do Movimento'));
...
where cl.custo_entidade_id = ce.id and
      ...
      Substituida essa linha do ano
      '|| to_char(ln_ano) ||' Ano, --ln_ano
*/

 ls_query:= 
 'select ''ORC'', q.tipo, q.ano, q.mes, q.empresa, q.uo, 
            q.centroresponsabilidadeseq, q.cta_orc, sum(q.qtde), 
            sum(q.valor), ''0'' fixo, 
            ''' || ls_tipo || '-'||to_char(ln_ano)||'.TXT'' Arquivo,
            sysdate ano_plano_contas, ''0'' Cta_contabil, 
            ''A'' Automatico, sysdate Dt_Atualizacao from 
     (select 
        case ''' || ls_tipo || '''
        ...
        to_number(to_char(cl.data,''mm'')) Mes,
     ...
     where b.titulo =  '''|| ls_tipo|| '-' || to_char(ln_ano) || ''' and 
           ...
           p.uo_id = u.id and
           cl.tipo =''P'' and
           to_number(to_char(cl.data,''yyyy'')) = ' || ln_ano  || ') q
      group by ano, mes, empresa, uo, centroresponsabilidadeseq, cta_orc
      order by ano, mes, empresa, uo, CentroResponsabilidadeSEQ, cta_orc ';
...
begin
   ...
   if vend=0 then
      dbms_output.put_line('O arquivo n?o possui um FIM DE LINHA do padr?o DOS/WINDOS (char 13 + char 10). Provavelmente e um arquivo no formato UNIX.');
   ...
   lb_erro:=false;
         if dados(2)='3' then -- Realizado
            ...
            -- 2. Atributo 2 deve ter 'YYYY XXXXXX' onde YYYY e o ano do orcamento e XXXXXX s?o os 6 primeiras posicoes do Centro de Resposabilidade
            ...
            begin
             select ci.categoria_item_id into ln_categ
              ...
              exception when no_data_found then
                ln_categ:=0;
                  lv_mensagem := 'Linha:'||chr(9)||registros||chr(9)||' Atributo Centro Responsabilidade n?o localizado: '||chr(9)||dados(3)||' '|| substr(dados(7),1,9);
            ...
            begin
             select u.id into ln_uo from uo u where to_number(substr(u.titulo,5,instr(u.titulo,'-')-6)) = to_number(dados(6));
              ...
              exception when no_data_found then
                ln_uo:=0;
                  lv_mensagem := 'Linha:'||chr(9)||registros||chr(9)||' UO n?o localizado: '||chr(9)||dados(3)|| ' ' || dados(6);
                  dbms_output.put_line(lv_mensagem);
                  SYS.UTL_FILE.put_line(lf_log, lv_mensagem);
                  lb_erro:=true;
                when others then
...
--dbms_output.put_line(dados(7)||'-'||ln_categ||'-'||ln_uo);
            ...
            where p.uo_id = ln_uo  and
                ...
                from atributoentidadevalor aev
                   where aev.identidade = p.id and
                         ...
                         aev.atributoid=1);
            exception when no_data_found then
                lv_mensagem := 'Linha:'||chr(9)||registros||chr(9)||' N?o foi localizado Projeto com Atributo Centro Responsabilidade: '||chr(9)||dados(3)|| ' ' || substr(dados(7),1,9) ||chr(9)|| ' Atributo_SEQ:'||chr(9)|| substr(dados(7),10)||chr(9) || ' UO: '||chr(9)||dados(6)||chr(9)|| ' COD_CR:'||chr(9)||ln_categ || chr(9)||' COD_UO:'||chr(9)||ln_uo;
                dbms_output.put_line(lv_mensagem);
                SYS.UTL_FILE.put_line(lf_log, lv_mensagem);
                lb_erro:=true;
              when others then
           ...
           if (not lb_erro) then
            ...
            if ln_ce > 0 then
                  ...
                  -- dbms_output.put_line('Ja existe Custo Lancamento para Lancamento para Conta:'||dados(14)||' Projeto:'|| '???' ||' Data:'||substr(dados(16),1,10)|| ' Linha n?o importada:'||registros);
               ...
               lv_mensagem := 'Linha:'||chr(9)||registros||chr(9)||'N?o foi localizado Custo Entidade para Conta:'||chr(9)||dados(8)||chr(9)||' Projeto:'||chr(9)||ln_proj;
               dbms_output.put_line(lv_mensagem);
               SYS.UTL_FILE.put_line(lf_log, lv_mensagem);
            end if;
            end if;
...
where id=ln_doc_cont;
 ...
 EXCEPTION
  ...
  WHEN UTL_FILE.invalid_path THEN
   ln_retorno:=-1;
   dbms_output.put_line('Diretorio Invalido. Abortado');
   return;
  ...
  WHEN UTL_FILE.invalid_filename THEN
   ln_retorno:=-1;
   dbms_output.put_line('Nome de Arquivo invalido. Abortado');
   return;
...
-----------------------------------------
-- ESTA PROC NAO ESTA SENDO UTILIZADA  --
-----------------------------------------
       ...
       if (nvl(trim(dados(11)),'-1') <> '-1') then
         ...
         loop
             if array(x)(2) in(73) then  -- DATA
                ...
                exception when no_data_found then
                  ln_categ:=0;
                    dbms_output.put_line('Projeto:'||ln_id_proj||'  Atributo Centro Responsabilidade n?o localizado: '||dados(array(x)(1)) );
                  when others then
                  ln_categ:=0;
...
*/
      ...
      INSERT INTO ATIVIDADE (ID, TITULO, MODIFICADOR,
       ...
       if (nvl(trim(dados(17)),'-1') <> '-1') then  -- Atributo Recomendac?es
...
--      dbms_output.put_line('Tit ini hor:'||dados(2) ||'-' || trim(dados(6))||'-' || trim(dados(10)));
   ...
   if dados(1)='50' then -- INICIO Metas
      ...
      ls_campo:='Descric?o:' || nvl(trim(dados(2)),' ');
      ...
      ls_campo:=ls_campo||'Criterio (Status ou Acumulado):'|| nvl(trim(dados(5)),' ')  || CHR(13) || CHR(10);
      ls_campo:=ls_campo||'Unidade de medida:'|| nvl(trim(dados(6)),' ') || CHR(13) || CHR(10);
      ls_campo:=ls_campo||'Ano:'|| nvl(trim(dados(7)),' ') || CHR(13) || CHR(10);
      ls_campo:=ls_campo||'mes limite:'|| nvl(trim(dados(8)),' ') || CHR(13) || CHR(10);
      ...
      ls_campo:=ls_campo||'Observac?es:'|| nvl(trim(dados(13)),' ') || CHR(13) || CHR(10);
   ...
   if dados(1) in ('60','90') then -- INICIO Despesas / Receitas
      ...
      exception
         when no_data_found then
         dbms_output.put_line('Projeto:'||ln_id_proj ||' N?o localizou na Arvore de custos:'||trim(dados(2)) || ' - ' || trim(dados(3))|| ' Tipo:'||ls_tipo);
         lb_erro:=true;
         when others then
         dbms_output.put_line('Projeto:'||ln_id_proj ||' Arvore de custos com mais de 1 registro para:'||trim(dados(2)) || ' - ' || trim(dados(3))|| ' Tipo:'||ls_tipo);
         lb_erro:=true;
      end;

      if not lb_erro then

      -- inclui permissoes automaticas para inclus?o de lancamento
      ln_id_atr:=0;
       select count(1)
         into       ln_id_atr
      ...
      if (nvl(trim(dados(8)),'-1') <> '-1') and to_number(dados(8))>0 then
         ...
         insert into custo_lancamento  (ID,CUSTO_ENTIDADE_ID,TIPO,SITUACAO,
                                        ...
                                        USUARIO_ID,DATA_ALTERACAO)
           select ln_id_tar,ln_id_atr,'R','V',  -- realizado, valido
      ...
      if (nvl(trim(dados(5)),'-1') <> '-1') and to_number(dados(5))>0 then
         ...
         insert into custo_lancamento  (ID,CUSTO_ENTIDADE_ID,TIPO,SITUACAO,
                                        DATA,VALOR_UNITARIO,QUANTIDADE,VALOR,
                                        USUARIO_ID,DATA_ALTERACAO)
           select ln_id_tar,ln_id_atr,'P','V',  -- realizado, valido
                  trunc(to_date(trim(dados(4)) ,'DD/MM/YYYY HH24:MI:SS')),
...
--     dados(1):='2010 1900101043504';
     ...
     select ci.categoria_item_id into ln_categ
      ...
      exception when no_data_found then
        ln_categ:=0;
          dbms_output.put_line('Linha:'||registros||' Atributo Centro Responsabilidade n?o localizado: '||dados(1) );
          lb_erro:=true;
        when others then
        ln_categ:=0;
    ...
    if not lb_erro then
     ...
     select Identidade into ln_id_proj
      ...
      exception when no_data_found then
        ln_categ:=0;
          dbms_output.put_line('Linha:'||registros||' N?o foi localizado Projeto com Atributo Centro Responsabilidade: '||dados(1) );
          lb_erro:=true;
        when others then
        ln_categ:=0;
...
--        dbms_output.put_line('Custo:'|| trim(dados(4))||' Proj:'|| ln_id_proj);
        ...
        exception
           when no_data_found then
           dbms_output.put_line('Linha:'||registros||' Projeto:'||ln_id_proj ||' N?o localizou na Arvore de custos:'|| trim(dados(4))|| ' Tipo:'||ls_tipo);
           lb_erro:=true;
           when others then
           dbms_output.put_line('Linha:'||registros||' Projeto:'||ln_id_proj ||' Arvore de custos com mais de 1 registro para:'|| trim(dados(4))|| ' Tipo:'||ls_tipo);
           lb_erro:=true;
        end;
...
--           dbms_output.put_line('Achou Tarefa');
        exception
           when no_data_found then
           dbms_output.put_line('Linha:'||registros||' Projeto:'||ln_id_proj ||' N?o Achou Tarefa:'||trim(dados(3)));
           ln_id_tarc:=0;
           lb_erro:=true;
           when others then
      ...
      begin
        ...
        exception
           when no_data_found then
           dbms_output.put_line('Linha:'||registros||' Projeto:'||ln_id_proj ||' N?o Achou Atividade:'||trim(dados(3)));
           ln_id_ativc:=0;
           lb_erro:=true;
           when others then
      ...
      if (nvl(trim(dados(7)),'-1') <> '-1') and to_number(dados(7))>0 then
         ...
         insert into custo_lancamento  (ID,CUSTO_ENTIDADE_ID,TIPO,SITUACAO,
                                        DATA,VALOR_UNITARIO,QUANTIDADE,VALOR,
                                        USUARIO_ID,DATA_ALTERACAO)
           select ln_id_tar,ln_id_atr,'P','V',  -- planejado, valido
                  trunc(to_date(trim(dados(6)) ,'DD/MM/YYYY HH24:MI:SS')),
...
--if registros < 1000 then
   ...
   if dados(1)='10' then -- PROJETOS
      ...
      where Titulo = dados(2)|| ' - ' || dados(3) and
           id >= (select id from projeto where titulo in ('PJ-NAC 1011 - Programa SENAI de Ac?es Inclusivas')) and
           id <= (select id from projeto where titulo in ('PJ-NAC 1028 - Apoio a estruturac?o de programa de capacitac?o de RH em normalizac?o - 2010')) and
      ...
      exception when no_data_found then
        ln_categ:=0;
          dbms_output.put_line('ERRO Linha:'||registros||' N?o localizou projeto: '||dados(2)|| ' - ' || dados(3) || ' CR:'||dados(11));
          lb_erro:=true;
          ln_id_proj:=0;
        when others then
        ln_categ:=0;
   ...
   if (dados(1)='30' and not lb_erro and ln_id_proj>0 ) then -- INICIO Ativ
      ...
      exception when no_data_found then
      ln_id_ativ:=0;
          dbms_output.put_line('ERRO Linha:'||registros||' N?o localizou Atividade: '||dados(2)|| ' - ' || dados(3) || ' CR:'||dados(11));
          lb_erro_ativ:=true;
          ln_id_proj:=0;
        when others then
          ln_id_ativ:=0;
   ...
   if (dados(1)='40' and not lb_erro and not lb_erro_ativ and ln_id_proj>0 ) then -- INICIO Tarefas
      ...
      exception when no_data_found then
        ln_categ:=0;
          dbms_output.put_line('ERRO Linha:'||registros||' N?o localizou tarefa: '||dados(2) );
        when others then
        ln_categ:=0;
...
ls_temp varchar2(10);
begin
   dbms_output.put_line('Horario: ' || to_char(sysdate, 'hh24:mi:ss dd/mm/yyyy'));
...
--falta analisar a questao do Debito - Credito
--falta descricao do lancamento (ficara em campo novo)
----------------------------------------------------------
--                                                      --
----------------------------------------------------------
         ...
         if dados(2)='3' then -- Realizado
            ...
            begin
             select u.id into ln_uo from uo u where u.titulo like dados(6)||'%';
              ...
              exception when no_data_found then
                ln_uo:=0;
                  lv_mensagem := 'Linha:'||chr(9)||registros||chr(9)||' UO n?o localizado: '||chr(9)||dados(6);
                  dbms_output.put_line(lv_mensagem);
                  SYS.UTL_FILE.put_line(lf_log, lv_mensagem);
                  lb_erro:=true;
                when others then
...
--dbms_output.put_line(dados(7)||'-'||ln_categ||'-'||ln_uo);
           ...
           if (not lb_erro) then
---Modificado por Márcia (estava procurando o titulo na coluna de custo_receita ao invés de custo_entidade

            select nvl(min(ce.id),0) into ln_ce from custo_entidade ce, custo_receita cr
            where ce.tipo_entidade = 'P' and
                  ce.custo_receita_id = cr.id and
                  ce.entidade_id = ln_proj and
                  ce.titulo = 'Realizado Zeus' and
                  cr.titulo like dados(8)||'%'; -- Realizado Zeus
            ...
            exception when no_data_found then
                lv_mensagem := 'Linha:'||chr(9)||registros||chr(9)||' N?o foi localizado Conta Custo Receita: '||chr(9)||dados(8);
                dbms_output.put_line(lv_mensagem);
                SYS.UTL_FILE.put_line(lf_log, lv_mensagem);
                lb_erro:=true;
              when others then
                lv_mensagem := 'Linha:'||chr(9)||registros||chr(9)||' Erro. A Conta Custo Receita deve ser unica: '||chr(9)||dados(8);
                dbms_output.put_line(lv_mensagem);
                SYS.UTL_FILE.put_line(lf_log, lv_mensagem);
                lb_erro:=true;
             end;
...
begin
  ...
  if pregra_id is null then
    ...
    while true loop
     ...
     if dom.tipo = 'C' then -- CRIADOR
     ...
     elsif dom.tipo = 'GRC' then -- GERENTE DE RECURSO DO CRIADOR
         select count(*) into contador
         from demanda de, usuario u
         where de.criador = u.usuarioid
         and de.demanda_id = dom.demanda_id
         and u.gerente_recurso = pusuario_id;
         ...
         if contador > 0 then
    ...
    elsif dom.tipo = 'GRS' then -- GERENTE DE RECURSO DO SOLICITANTE
          select count(*) into contador
           from demanda de, usuario u
           where de.solicitante = u.usuarioid
           and de.demanda_id = dom.demanda_id
   ...
   elsif dom.tipo = 'RUE' then -- RESPONSAVEL DO ATRIBUTO USUARIO ESPECIFICO DA DEMANDA
           select count(*) into contador
           from demanda de, regra_formulario rf, estado_formulario ef, atributo_valor av, usuario u
           where rf.formulario_id = de.formulario_id
           and ef.formulario_id = de.formulario_id
           and ef.regra_id = nvl(pregra_id, rf.regra_id)
           and ef.estado_id = de.situacao
           ...
           and av.atributo_id = rf.atributo_id(+)
           and de.demanda_id(+) = av.demanda_id
           and u.usuarioid = pusuario_id
           and av.valor = u.responsavel_id;
          ...
          if contador > 0 then
     ...
     elsif dom.tipo = 'GUE' then -- GERENTE DE RECURSO DO ATRIBUTO USUARIO ESPECÍFICO
           select count(*) into contador
           from demanda de, regra_formulario rf, estado_formulario ef, atributo_valor av, usuario u
           where rf.formulario_id = de.formulario_id
           and ef.formulario_id = de.formulario_id
           ...
           and de.demanda_id(+) = av.demanda_id
           and u.usuarioid = pusuario_id
           and av.valor = u.gerente_recurso;
     ...
     elsif dom.tipo = 'UE1' then -- 1º RESPONSAVEL DA UNIDADE ORGANIZACIONAL DO USUARIO ESPECIFICO
          select count(*) into contador
          from demanda de, regra_formulario rf, estado_formulario ef, atributo_valor av, uo, usuario u
          where rf.formulario_id = de.formulario_id
          and ef.formulario_id = de.formulario_id
          and ef.regra_id = nvl(pregra_id, rf.regra_id)
          and ef.estado_id = de.situacao
     ...
     elsif dom.tipo = 'UE2' then -- 2º RESPONSAVEL DA UNIDADE ORGANIZACIONAL DO USUARIO ESPECIFICO
          select count(*) into contador
          from demanda de, regra_formulario rf, estado_formulario ef, atributo_valor av, uo, usuario u
          where rf.formulario_id = de.formulario_id
          and ef.formulario_id = de.formulario_id
     ...
     elsif dom.tipo = 'RRA' then -- RESPONSAVEL DO RESPONSAVEL DE ATENDIMENTO DA DEMANDA
          select count(*) into contador
          from usuario u, demanda de
          where de.demanda_id = dom.demanda_id
          and de.responsavel = u.usuarioid
          and u.responsavel_id = pusuario_id;
     ...
     elsif dom.tipo = 'GRA' then -- GERENTE DE RECURSO DO RESPONSAVEL DE ATENDIMENTO DA DEMANDA
          select count(*) into contador
          from usuario u, demanda de
          where de.demanda_id = dom.demanda_id
          and de.responsavel = u.usuarioid
     ...
     elsif dom.tipo = 'RA1' then -- 1º RESPONSAVEL DA UNIDADE ORGANIZACIONAL DO RESPONSAVEL DE ATENDIMENTO DA DEMANDA

          select count(*) into contador
          from usuario u, demanda de, uo
          where de.demanda_id = dom.demanda_id
          and de.responsavel = u.usuarioid
          and u.uo_id = uo.id
          and uo.responsavel = pusuario_id;
    ...
    elsif dom.tipo = 'RA2' then -- 2º RESPONSAVEL DA UNIDADE ORGANIZACIONAL DO RESPONSAVEL DE ATENDIMENTO DA DEMANDA
    ...
    elsif dom.tipo = 'DEP' then -- MEMBROS DO DIRETÓRIO DA EQUIPE DOS PROJETOS ASSOCIADOS A DEMANDA

        select count(*) into contador from dual
        where pusuario_id in (select distinct usuarioid
                              from papelprojetorecurso
                              where projetoid in ( select identidade
                                                   from solicitacaoentidade
                                                   where tipoentidade = 'P'
                                                   and solicitacao in ( select demanda_id from demanda
                                                                  start with demanda.demanda_id = dom.demanda_id
                                                                  connect by prior demanda.demanda_pai = demanda.demanda_id
                                                                  )
                                                  )
        );
  ...
  for contador2 in 1..idDemandas.count loop
    for dom in (select de.demanda_id, te.obrigar_documento, te.obrigar_ter_documento, te.obrigar_entidade, te.obrigar_ter_entidade from transicao_estado te, demanda de
             ...
             if dom.obrigar_ter_entidade = 'S' then
               select max(qtd) into contador from (
                                     select count(*) as qtd
                                     from solicitacaoentidade
                                     ...
                                     from demanda
                                     where demanda.demanda_pai = dom.demanda_id);
             ...
             if dom.obrigar_entidade = 'S' then
               ...
               select max(qtd) into contador from (
                                                   select count(*) as qtd from demanda_entidade_estado de
                                                   where de.demanda_id = dom.demanda_id
                                                   union
                                                   select count(1) as qtd
                                                   from demanda
  ...
  open lc_sql for lv_sql;
    while true loop
      ...
      exit when lc_sql%notfound;
            ...
            exception when others then
              a_dom.dominio_atributo_id := null;
              a_dom.valor := null;
              a_dom.valordata := null;
              a_dom.valornumerico := null;
              a_dom.valor_html := null;
              a_dom.categoria_item_atributo_id := null;
              a_dom.matriz_id := null;
            end;
...
create or replace package body pck_regras is
   ...
   */
   ...
   ln_count_select                 number;
   type t_cursor is ref cursor;
   lc_cursor_atr_val t_cursor; 
   lv_atr_val                      varchar2(32000);
             ...
             if c.tipo_valor in ('atributo') then
               ...
               if pb_append then
                     ...
                     if lv_nome_tabela = 'ATRIBUTO_VALOR' then
                       ...
                       /* lv_atr_val := ' select count(1) from ATRIBUTO_VALOR '||
                                      ' where demanda_id = '||ln_id ||  
                                      ' and atributo_id = '|| ln_atributo_id ||
                                      ' and ATRIBUTO_VALOR_ID in ('||lv_sql_destino||')' ;
                        */
                        --open lc_cusor_atr_val for lv_atr_val;
                        --fetch lc_cusor_atr_val into ln_count_select;
                        --close lc_cusor_atr_val;
                        ln_count_select := null;
                        open lc_cursor_atr_val for lv_sql_destino;
        
                        while true loop
                           fetch lc_cursor_atr_val into ln_count_select;
                           exit when lc_cursor_atr_val%notfound;
                        end loop;
                        close lc_cursor_atr_val;
                        
                        if ln_count_select is null then
                          if ln_atributo_id is null then
                              if pn_propriedade_id_destino is not null then
                                select max(atributo_id) into ln_atributo_id 
                                from regras_propriedade_niveis 
                                where propriedade_id = pn_propriedade_id_destino
                                and atributo_id is not null;
                              else
                                select max(atributo_id) into ln_atributo_id 
                                from regras_propriedade_niveis 
                                where propriedade_id = pn_propriedade_id_origem
                                and atributo_id is not null;                                
                              end if;
                          ...
                          if ln_atributo_id is not null then
                             lv_atr_val := 'INSERT INTO ATRIBUTO_VALOR ' ||
                                        '(ATRIBUTO_ID, DEMANDA_ID, DATE_UPDATE, USER_UPDATE,' ||
                                        c.coluna || ') '||
                                        ' VALUES ('||
                                        ln_atributo_id || ','||
                                        pn_demanda_id  || ',sysdate,''' ||
                                        pv_usuario_id || ''',' ||
                                        lv_valor_atualizar || ')';
                             execute immediate lv_atr_val;
                           end if;             
                        end if;
     ...
     begin
         ...
         if rec_acao.chave_campo is not null then
            if rec_acao.chave_campo = 'DESTINO' then
               ...
               if rec_acao.valor_troca is not null then
                  select descricao into lv_valor_troca from destino where destinoid = to_number(replace(rec_acao.valor_troca, 'D',''));
               end if;
           ...
           if acao.secao_atributo_id is not null then
                 ...
                 where atributoid = rec_secao_atributo.atributo_id;
                    ...
                    if pck_atributo.Tipo_LISTA_MULTIPLA_ESCOLHA = rec_atributo.tipo or
                       ...
                       elsif pck_atributo.Tipo_MONETARIO = rec_atributo.tipo or
                             pck_atributo.Tipo_NUMERO = rec_atributo.tipo then
                             if trim(lv_valor_troca) is not null then
                                update atributo_valor
                                
                                set valornumerico = to_number(replace(replace(lv_valor_troca,'.',''),',','.'),'99999999999999990D9999999999999999','NLS_NUMERIC_CHARACTERS =''.,'''),
                                 date_update = sysdate,
                                 user_update = pv_usuario
                                where demanda_id = rec_demanda.demanda_id
                                and   atributo_id = rec_atributo.atributoid;
                      ...
                      /* elsif pck_atributo.Tipo_NUMERO = rec_atributo.tipo  then
                             if trim(lv_valor_troca) is not null then
                                update atributo_valor
...
insert into versao(id, titulo, patch, versao_tgp_id, comentario)
       values (versao_seq.nextval, '6.0.1', '02', 4, 'Aplicação de Patch');
commit;
/
