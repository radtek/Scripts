...
alter table mapa_indicador_quebra add vigente varchar2(1)default 'Y' not null;
...
( select mi.id id, 'I' TIPO, indicador_template ,mi.entidade_id ENTIDADE_ID, mi.tipo_entidade TIPO_ENTIDADE,
       ...
       mia.escore - mm.valor / mm.valor DIF_PERC,
       mia.escore / mm.valor PERC_META_ATING,
       mmf.cor COR
   ...
   and ( ( mmf.percentual_meta is null ) or
         ( mmf.percentual_meta = nvl( (select min(mmf2.percentual_meta)
                                        ...
                                        where mmf2.indicador_id = mm.indicador_id
                                          and mmf2.percentual_meta >= nvl((mia.escore / mm.valor)*100,0))
                                     , mmf.percentual_meta)))
union all
select mo.id id, 'O' TIPO,'N' indicador_template, mo.entidade_id ENTIDADE_ID, mo.tipo_entidade TIPO_ENTIDADE,
       ...
       moa.escore - mom.valor / mom.valor DIF_PERC,
       moa.escore / mom.valor PERC_META_ATING,
       mof.cor COR
   ...
   and ( ( mof.percentual_meta is null ) or
         ( mof.percentual_meta = nvl( (select min(mof2.percentual_meta)
                                        ...
                                        where mof2.objetivo_id = mom.objetivo_id
                                          and mof2.percentual_meta >= nvl((moa.escore / mom.valor)*100,0))
                                     , mof.percentual_meta)))
);
/
...
create or replace package body pck_indicador is
   ...
   begin
      ...
      for c in (select *
         ...
         if c.tipo = 'M' and --Manual
            c.subtipo = 'M' and --Matematico
            c.situacao = 'E' then
...
-------------------------------------------------------------------------------
delete from secao_atributo 
where regra_id not in 
               (select regra_id from regra_formulario where 
                       regra_formulario.formulario_id = secao_atributo.formulario_id 
...
insert into versao(id, titulo, patch, versao_tgp_id, comentario)
       values (versao_seq.nextval, '5.2.0', '22', 2, 'Aplicação de Patch');
commit;
/
