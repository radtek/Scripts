...
select table_owner, table_name, num_rows, round(blocks * (select block_size from dba_tablespaces where tablespace_name = tp.tablespace_name) / 1024 / 1024 ) as sizeMB, tablespace_name, partition_position, high_value, partition_name, 
	DECODE((select 1
			from dba_tab_partitions tb_TMP
				 inner join dba_data_files df_TMP
					on df_TMP.tablespace_name = tb_TMP.tablespace_name
			where df_TMP.STATUS = 'AVAILABLE'
		...
		(select column_name from DBA_PART_KEY_COLUMNS where owner = tp.TABLE_OWNER and NAME = tp.TABLE_NAME AND OBJECT_TYPE = 'TABLE') AS COLUMN_NAME, 
