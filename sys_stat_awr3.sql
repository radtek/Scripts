...
prompt apenas soma o valor, ideal para contadores 
prompt ***************
...
select STAT_NAME, 
       ...
       round(value - LAG(value, 1, 0) OVER (PARTITION BY startup_time, STAT_NAME  ORDER BY snap_id, startup_time)) as delta,
	   round(value) accum_sec,
       snap_id, 
	   startup_time
from (  select ss.startup_time, ss.snap_id, ss.END_INTERVAL_TIME, hst.STAT_NAME, sum(hst.value) value
