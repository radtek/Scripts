...
and sql.hash_value in (&hash_value)
--and   sql.hash_value = &hash_value
and to_char(sql.child_number) like '&childnumber'
order by
    sql.hash_value asc,
...
and p.hash_value in (&hash_value)
--and   p.hash_value = &hash_value
and to_char(p.child_number) like '&childnumber'  -- to_char is just used for convenient filtering using % for all children
order by
    p.hash_value asc,
    p.child_number asc,
    p.id asc    
...
from (
    ...
    and to_char(child_number) like '&childnumber'
...
undef hv;
col VALUE_STRING for a30 wrapped;
col DATATYPE_STRING for a15 wrapped;
col name for a20;
col POSITION for a10 wrapped;
col sql_id for a10 wrapped;

select sql_id, POSITION, name, VALUE_STRING, DATATYPE_STRING,CHARACTER_SID
...
SELECT
         t.sql_text
FROM     v$sqltext t
WHERE    t.hash_value = &hash_value
ORDER BY t.piece
;

undefine hash_value 
undefine childnumber 
