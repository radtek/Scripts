...
from v$session a
   ...
   left join v$session_wait w
           on a.sid = w.sid
where a.username not like upper('LINK')
  and a.status = 'ACTIVE'
...
from v$session a
   ...
   left join v$session_wait w
           on a.sid = w.sid
where a.username like upper('LINK')
  and a.status = 'ACTIVE'
order by "Idle - min" desc, a.username, a.status;
...
select sum(case when status = 'ACTIVE' and username = 'LINK' then 1 else 0 end) active_LINK, 
       sum(case when status <> 'ACTIVE' and username = 'LINK' then 1 else 0 end) idle_LINK,
       sum(case when status = 'ACTIVE' and username <> 'LINK' then 1 else 0 end) active_outros,
       sum(case when status <> 'ACTIVE' and username <> 'LINK' then 1 else 0 end) idle_outros
from v$session;
