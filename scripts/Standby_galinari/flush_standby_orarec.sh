...
LOG=$SCRIPT_HOME/logs/dc2flush_orarec_`date +%Y%m%d%H%M`.log

. /home/oracle/orarec.env
echo "Start Flush at: "`date +%Y%m%d_%H%M` >> $LOG
DTF_PRD=`sqlplus -S system/madaga5car@ORARECDC1 <<EOF
...
ST1=`sqlplus -S system/madaga5car@ORARECDC1 <<EOF
set head off feedback off
select max(sequence#) from v\\\$archived_log where deleted='YES' and thread#=1;
exit
EOF`

ST2=`sqlplus -S system/madaga5car@ORARECDC1 << EOF
set head off feedback off
