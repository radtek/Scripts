####################################################################################################################
# Script que realiza export da particao mais antiga e manda pra backup morto em fita (netbackup).
# Caso esse procedimento ocorra com sucesso, o script ja dropa a particao processada.
# O script recebe 2 parametros:
# 1) Owner
# 2) Nome da tabela
# Ele verifica na system.particao_tab_config no campo NRO_PARTICOES_READ_ONLY.
# Se a tabela tiver mais Particoes Read only que esse nro ele exporta essas particoes.
####################################################################################################################
...
ARQ_SAIDA="lista_part_export.${DATA_ATUAL}"
BDUSR=BKP
BDPAS=BKPORAACCT2
...
if [ -f  ${DIR_SAIDA}/${ARQ_SAIDA} ] ; then
  echo "Antes do List"
  LIST_PAR=`cat ${DIR_SAIDA}/${ARQ_SAIDA}`
  echo "Lista:$LIST_PAR"
  for L in ${LIST_PAR}
  do
        ...
        if [ ! $? = 0 ] ; then
           ...
           ## Se o bkp pra fita foi Ok, entao dropa a particao
           $WORK_DIR/backup/drop_old_partition.sh ${OWNER} ${TABLE} ${L}
           if [ ! $? = 0 ] ; then
              echo "`date ` => Erro encontrado no drop partition: ${OWNER} ${TABLE} ${L}"
              exit 6
           fi
        fi
        ######
