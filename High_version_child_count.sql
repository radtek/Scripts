...
--> ver qual mutex é o gargalo:
	select * from X$MUTEX_SLEEP order by sleeps

----> workaround que resolver para DIMED:
	Bug 18693124  High version_count for SQL with remote PL/SQL operand
	alter system set events '22827 trace name context forever, level 1';
	
	----> GEROU OUTRO TIVEMOS QUE REVER e APLICAR O PATCH DO Bug 18693124:
		ORA-02019 ORA-2083 ORA-2063 DURING REMOTE INSERT (Doc ID 1484079.1)
		
----> workaroud enviado pela oracle
	"_cursor_obsolete_threshold" set to 25

exec sys.DBMS_SHARED_POOL.PURGE ('00000018AEEFA560, 2805680925', 'C');
	
	
--- validar conforme o motivo de não compartilhamento
	VIEW: "V$SQL_SHARED_CURSOR" Reference Note (Doc ID 120655.1

	------- List top sql com high versions
		select * from (select count(1) ctd, sql_id, parsing_schema_name, IS_OBSOLETE from v$sql group by sql_id, parsing_schema_name,IS_OBSOLETE order by ctd desc) where rownum < 10 order by ctd desc

----> setup event via oradebug
sqlplus / as sysdba
oradebug setospid 3654
oradebug SESSION_EVENT 22827 trace name context forever, level 1
oradebug eventdump session
oradebug tracefile_name	

oradebug setospid 3654
oradebug SESSION_EVENT 22827 trace name context off
oradebug eventdump session
oradebug tracefile_name	

--> para ver output verificar arquivo de trace

---> Podemos definir o limite de childs ativos:
...
Alterado parametro "optimizer_secure_view_merging" para FALSE conforme notas:
-> Bug 16809786  Secure view merging does not occur when user defined operators are present
-> Bug 11930680  High VERSION_COUNT due to AUTH_CHECK_MISMATCH / INSUFF_PRIVS with secure view merging

select sid,serial#,SQL_ID,BLOCKING_SESSION,BLOCKING_SESSION_STATUS,EVENT, (select sql_id from v$session s2 where s2.sid = s.BLOCKING_SESSION) as sql_id_b, (select event from v$session s2 where s2.sid = s.BLOCKING_SESSION) as event_b
     from v$session s where event ='cursor: pin S wait on X'

	 
select * from (select count(1) ctd, sql_id, parsing_schema_name from v$sql group by sql_id, parsing_schema_name order by ctd desc) where rownum < 20 order by ctd

    CTD         SQL_ID PARSING_SCHEMA_NAME
------- -------------- ------------------------------
     26 6adf722ju87qq  PROCOR
     29 459f3z9u4fb3u  SYS
     31 4mzhq7gzjjz96  DIMEDWEB
     40 2qtf76kh3y0ky  DIMEDWEB
     44 2tkw12w5k68vd  SYS
     51 3nkd3g3ju5ph1  SYS
     56 7ng34ruy5awxq  SYS
    179 bbkbg2mmbbskf  DIMEDWEB
    247 0ah0b6v4jr1xw  PROBTB3
    322 fjy8wxj9c89js  DIMEDWEB
    356 3vfn87q177vsh  DIMEDWEB
    421 9jfaja0qrza0n  DIMEDWEB
    441 9khnpcq6s1dqc  DIMEDWEB
    568 5pfx87mfpx0kp  DIMEDWEB
    680 95wt3z5hg73rz  DIMEDWEB
    885 5pza3gtfdc8kz  MADORNES
    885 98b0xakamc0ur  MADORNES
   1074 3w1dhhmcmbkcf  DIMEDWEB
   2074 gyjhwskmmqksx  DIMEDWEB	 
	 


---> PARA USAR:
	rem select b.* from v$sqlarea a ,table(version_rpt(a.sql_id,null,'Y')) b where loaded_versions >=100 and sql_id = '&sql_id';
...
rem select b.* from v$sqlarea a ,table(sys.version_rpt(a.sql_id,null,'Y')) b where loaded_versions >=100 and sql_id=&sqlid;
