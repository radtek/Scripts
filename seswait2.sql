...
col p1text for a9  justify left
col p2text for a9  justify left
col p3text for a9  justify left
col seconds_in_wait for 99999999 heading (s)
col client_identifier format a15
col logon_time format a20
col program format a20
col WAITING_SINCE for a20 justify left

select /*+ rule */ 
       s.sid,s.username,s.osuser, s.machine, s.sql_id, w.seconds_in_wait as "SEC WAIT", 
       case when s.status = 'ACTIVE' then round(last_call_et/60) else -1 end "IDLE_MIN", 
       w.event,substr(w.p1text,1,9) p1text,w.p1, w.p2text,w.p2,w.p3text,w.p3,s.sql_hash_value,
	s.program,to_char(s.logon_time, 'dd/mm/rrrr hh24:mi') logon_time, 
	s.client_identifier, s.logon_time, s.sql_child_number, 
