...
prompt Ideal para valores divididos por 1000, ou seja, miliseconds ou valores muito grandes 
prompt ***************
...
select STAT_NAME, 
       ...
       round(value_k - LAG(value_k, 1, 0) OVER (PARTITION BY startup_time, STAT_NAME  ORDER BY snap_id, startup_time)) as delta_k,
	   round(value_k) accum_sec,
       snap_id, 
	   startup_time
from (  select ss.startup_time, ss.snap_id, ss.END_INTERVAL_TIME, hst.STAT_NAME, sum(hst.value / 1000) value_k
